...
sprite sheet (~16 KB) as a committed asset, and **leave `glyphs` unset on the generated style**,
which makes MapLibre rasterise every label locally with TinySDF. A `glyphs` URL can be set
opt-in per layer in `maplibreConfig.json` for deployments that run their own glyph server.
...
- `@protomaps/basemaps` becomes a runtime dependency of the main bundle.

...
- Arabic shaping and bidirectional ordering use the bundled RTL text plugin, registered
  during map initialization. The hosted build emits its script and embedded WebAssembly as a
  separate cacheable asset. The standalone build carries them in a data URL: workers cannot
  fetch a page's Blob URL from the opaque origin of a `file://` page. Neither build needs a
  plugin CDN, and both retain eager loading.

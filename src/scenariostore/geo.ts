...
export function useGeo(store: NewScenarioStore) {
  ...
  const asFeature = (item: NScenarioLayerItem | ScenarioLayerItem): NScenarioFeature =>
    item as unknown as NScenarioFeature;
  const asFeatures = (
    items: Array<NScenarioLayerItem | ScenarioLayerItem>,
  ): NScenarioFeature[] => items as unknown as NScenarioFeature[];
  ...
  function addFeatureStateGeometry(
    ...
    update(
      (s) => {
        const u = s.itemMap[featureId] as NScenarioFeature;
        const t = atTime ?? s.currentTime;
        newState = { t, geometry };
        if (t === s.currentTime) u._state = newState;
        if (!u.state) u.state = [];
  ...
  function addLayer(data: NScenarioLayer) {
    const itemIds = data.items ?? data.features ?? [];
    ...
    newLayer.features = newLayer.items;
    ...
    update(
      (s) => {
        ...
        layer.items.forEach((fid, i) => {
          const feature = s.itemMap[fid] as NScenarioFeature;
  ...
  function reorderFeature(
    ...
    const feature = state.itemMap[featureId] as NScenarioFeature;
    const destinationFeature = state.itemMap[destinationFeatureOrLayerId] as
      | NScenarioFeature
      | undefined;
  ...
  function getFullLayer(layerId: FeatureId): ScenarioLayer | undefined {
    const layer = getFullLayerItemsLayer(layerId);
    if (!layer) return;
    return { ...layer, features: asFeatures(layer.items) };
  }
  ...
  function getFullLayerItemsLayer(
    ...
    return {
      ...layer,
      items: layer.items.map((f) => klona(state.itemMap[f])),
    };
  }
  ...
  const layerItemsLayers = computed<ScenarioLayerItemsLayer[]>(() => {
    return state.layers
      ...
      .map((layer) => ({
        ...layer,
        items: layer.items.map((featureId) => state.itemMap[featureId]),
      }));
  });

  const layers = computed(() => {
    return layerItemsLayers.value.map((layer) => ({
      ...layer,
      features: asFeatures(layer.items),
    }));
  });
  ...
  const layersFeatures = computed(() => {
    return layersItems.value.map(({ layer, items }) => ({
      layer,
      features: asFeatures(items),
    }));
  });
    ...
    if (undoable) {
      update(
        (s) => {
          ...
          Object.assign(layer, normalizedData);
  ...
  function deleteLayer(layerId: FeatureId, options: UpdateOptions = {}) {
    ...
    update(
      (s) => {
        const layer = s.layerMap[layerId];
        if (!layer) return;
        layer.items.forEach((featureId) => delete s.itemMap[featureId]);
  ...
  function addFeature(
    ...
    update(
      (s) => {
        const layer = s.layerMap[layerId];
        if (!layer) return;
        s.itemMap[newFeature.id!] = newFeature;
    ...
    if (!noEmit) {
      featureLayerEvent
        .trigger({
          type: "addFeature",
          id: newFeature.id,
          data: newFeature as unknown as NScenarioFeature,
        })
        .then();
    }
  ...
  function deleteFeature(featureId: FeatureId, options: UpdateOptions = {}) {
    const noEmit = options.noEmit ?? false;
    const feature = state.itemMap[featureId];
    if (!feature) return;
    update(
      (s) => {
        const layer = s.layerMap[feature._pid];
        delete s.itemMap[featureId];
  ...
  function updateFeature(
    ...
    if (undoable) {
      update(
        (s) => {
          const feature = s.itemMap[featureId] as NScenarioFeature;
          if (!feature) return;
      ...
      const layer = state.itemMap[featureId];
      Object.assign(layer, data);
    }
    if (data.state) {
      updateFeatureState(featureId);
  ...
  function deleteFeatureStateEntry(featureId: FeatureId, index: number) {
    update((s) => {
      const _feature = s.itemMap[featureId];
      if (!_feature) return;
  ...
  function getLayerItemById(id: FeatureId) {
    ...
    if (!item) return { item, layer: undefined };
    return { item, layer: state.layerMap[item._pid] };
  }
  ...
  const itemsInfo = computed<LayerFeatureItem[]>(() => {
    ...
    layerItemsLayers.value.forEach((layer) => {
      ...
      const mappedFeatures: LayerFeatureItem[] = layer.items.map((item) => {
        const feature = asFeature(item);
  ...
  return {
    ...
    getFeatureById: (id: FeatureId) => {
      const { item, layer } = getLayerItemById(id);
      return { feature: item as unknown as NScenarioFeature, layer };
    },
    getLayerItemById,
    moveFeature,
    updateLayer,

...
export function toReferenceFeatureSelection(
  ...
  delete properties.__kmlDescription;
...
export function createMapLibreKmlLayerRenderer(
  ...
  function removeLayer(layerId: string) {
    ...
    const active = activeLayers.get(layerId);
    if (active) {
      for (const imageId of active.iconImageIds) {
        try {
  ...
  async function addLayer(layer: ScenarioKMLLayer) {
    ...
    const iconImageIds = previous?.iconImageIds ?? new Set<string>();
    ...
    try {
      ...
      activeLayers.set(layerId, { layer, data, loadToken, iconImageIds });
  ...
  async function registerIcons(layerId: string, data: ParsedKmlLayerData) {
    await Promise.all(
      [...data.icons].map(async ([imageId, href]) => {
        if (mlMap.hasImage(imageId)) {
          activeLayers.get(layerId)?.iconImageIds.add(imageId);
          return;
        }
        try {
